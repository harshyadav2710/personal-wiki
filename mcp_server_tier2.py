...
Has access to Tier 2 + Tier 3 (NOT Tier 1). Launched with MCP_TIER=2.
"""
...
port = int(os.getenv("MCP_TIER2_PORT", os.getenv("PORT", "10002")))

mcp = FastMCP(
    "recall-personal-wiki-tier2",
    host="0.0.0.0",
    port=port,
)
...
def search_tiered_wiki(query: str) -> str:
    ...
    results = search_tiered(query, limit=50)
    if not results:
...
def read_tiered_note(note_id: int) -> str:
    """Read a single note by id if it is in Tier 2 or Tier 3."""
    note = get_tiered(note_id)
    if not note:
        return "Note not found or not accessible from this tier server (Tier 1 is hidden)."
    return _format_note(note)


@mcp.tool()
def list_recent_tiered_notes(limit: int = 10) -> str:
    """List recent notes in Tier 2/3."""
